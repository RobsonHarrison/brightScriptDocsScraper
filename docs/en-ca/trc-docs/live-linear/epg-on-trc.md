...
**Tip** : Including Gracenote TMS ids or IMDB ids in your program metadata generally provides all the required metadata for any content type (see [A note about deduplication of metadata](https://developer.roku.com/en-ca/trc-docs/live-linear/epg-on-trc.md#a-note-about-deduplication-of-metadata)). TRC also supports channel level Gracenote TMS ids (aka ProgSrvIDs) but only for channels with live sports at this time. Ask your Roku rep for details.
