...
Name | Description
---|---
[roSystemLog](https://developer.roku.com/docs/references/brightscript/components/rosystemlog.md "roSystemLog") | The roSystemLog component enables the application to receive events from the Roku Streaming Player that are intended for reporting errors and trends, rather than trigger a response to a user action
## Supported methods
...
---|---|---
logType | String | The logType to be enabled, which may be one of the following values:  | Valid logTypes | Description
---|---
...
"http.error" | Sent whenever an error occurs while executing an HTTP request. This may be sent during the time of the initial connection for two possible reasons:
...
---|---|---
LogType | String | When enabled, the “http.complete” events will be sent to Roku after an http transfer is completed for adaptive streams. This event consolidates information related to:
...
See [roSystemLogEvent](https://developer.roku.com/docs/references/brightscript/events/rosystemlogevent.md "roSystemLogEvent") for details on the information reported for each log type.