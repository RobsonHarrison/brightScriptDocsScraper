...
When your app is tested and ready to deploy, you publish it using the Developer Dashboard. There are two types of app you can work with.
  * **[Beta app](https://developer.roku.com/en-gb/docs/developer-program/publishing/channel-publishing-guide.md#beta-channel-guidelines)** : A sharable app for testing without certification. A beta app is a temporary app for development purposes that cannot be published to the public Streaming Store.

  * **[Public app](https://developer.roku.com/en-gb/docs/developer-program/publishing/channel-publishing-guide.md#public-channel-guidelines)** : An app available for install from the Streaming Store. A public app must pass Roku's certification process and be reviewed by Roku before being published.
...
> In other words, when publishing an app, make sure that your own account is the **root account** associated with the app, and that your third-party developer has simply been granted the role of "Administrator" or "Channel management." Our document on [user access management in the Developer Dashboard](https://developer.roku.com/docs/features/dashboard/user-access-management.md) provides more information on root accounts and role-based permissions.
## App creation overview
When you create an SDK app, you'll have the option of setting the app as a beta or public app. For either type of app, you'll start by [packaging your Roku app](https://developer.roku.com/docs/developer-program/publishing/packaging-channels.md). Then follow the guidelines for creating a beta app or a public app.
...
Beta app usage is **limited** to its intended purpose of quality assurance, as follows:
  * **120 days.** A beta app can exist for only 120 days after you create it (using [Add Channel](https://developer.roku.com/en-gb/docs/developer-program/publishing/channel-publishing-guide.md#create-a-channel)). After the expiration of that period, the app is: (1) deleted and removed from your Developer Dashboard and (2) disabled for all users who have installed it.
...
To create and start testing a beta app, you will need to:
  1. [Package your Roku app](https://developer.roku.com/docs/developer-program/publishing/packaging-channels.md).
  2. [Create an app](https://developer.roku.com/en-gb/docs/developer-program/publishing/channel-publishing-guide.md#create-a-channel) (select **Beta**).
  3. [Upload the package](https://developer.roku.com/en-gb/docs/developer-program/publishing/channel-publishing-guide.md#upload-a-package) associated with this app.
...
To ensure success in creating a Public app, you will need to:
  1. [Package your Roku app](https://developer.roku.com/docs/developer-program/publishing/packaging-channels.md).
  2. Create your design assets following [Design and User Experience Guidelines](https://developer.roku.com/docs/developer-program/design/design-overview.md).
  3. Go through the [Pre-certification Check List](https://developer.roku.com/docs/developer-program/certification/certification.md).
  4. [Create an app](https://developer.roku.com/en-gb/docs/developer-program/publishing/channel-publishing-guide.md#create-a-channel) (select **Public**).
  5. [Upload the package](https://developer.roku.com/en-gb/docs/developer-program/publishing/channel-publishing-guide.md#upload-a-package) associated with this app.
  6. Follow the [App certification](https://developer.roku.com/en-gb/docs/developer-program/publishing/channel-publishing-guide.md#channel-certification) procedure.
  7. Publish your app (submit it for deployment) using the [Publishing an app](https://developer.roku.com/en-gb/docs/developer-program/publishing/channel-publishing-guide.md#publishing-a-channel) procedure.
...
Field | Description
---|---
Channel name | Enter a name for your app. The name may be a maximum of 30 characters, and it may include alphanumeric, special, and UTF-8 characters. This name will be displayed in your app's listing in the Streaming Store.
...
Channel store countries | Select the countries where your app will be distributed from the drop-down list. You can scroll through the list and select countries or enter the name of a country to filter the list as you type.
...
Field | Description
---|---
Channel type | Select the type of app you are creating:
...
The **Channel ID** and **Bundle ID** values identify an app to the publisher's ad partners in the advertising ecosystem. These values can also be found via the meta-tags on the app's Streaming Store page. Following IAB App-Ads guidelines, Roku uses the meta-tags `appstore:store_id` and `appstore:bundle_id` to provide channel ID information in the http header of an app's Streaming Store page. Using the values from the illustration above as examples, the meta-tags would appear in the `<head>` section, as so:```
...
Field | Description
---|---
Upload your .pkg or .zip file | Upload the package file (.pkg) file that you created with the [Development Application Installer](https://developer.roku.com/docs/developer-program/publishing/packaging-channels.md).
Channel version | Select the version number for your app.
Minimum firmware | Roku supports multiple [package file formats](https://developer.roku.com/docs/developer-program/publishing/packaging-channels.md). Each package format requires a different minimum firmware version to be run on Roku devices, based on when the format was introduced to the OS. Enter a minimum firmware version that is equal to or greater than the one required for the package format used by your app. | Package format | Minimum Roku OS version
---|---
zip | 5.2
cramfs | 7.7
squashfs | 8.0
...
### Deep linking
To validate that your app is handling deep link requests properly, provide at least one set of [deep linking parameters](https://developer.roku.com/docs/developer-program/discovery/implementing-deep-linking.md) for each different [media type](https://developer.roku.com/docs/developer-program/discovery/implementing-deep-linking.md) in your app. For example, if your app contains movies and TV episodes, include deep linking parameters for at least one movie and one episode.
...
Certification Requirement | Provides a link to any related certification requirements in the [Certification Criteria](https://developer.roku.com/docs/developer-program/certification/certification.md) document.

For additional usage details and operational information, including a list of the tests conducted, see the [Static Analysis Tool](https://developer.roku.com/docs/developer-program/dev-tools/static-analysis-tool/static-analysis-tool.md) documentation.
### Test credentials
...
> To run the app launch performance test, your app must have the required [AppLaunchComplete signal beacon](https://developer.roku.com/docs/developer-program/performance-guide/measuring-channel-performance.md).
...
![channel-builder-2-cba-selection.png - roku600px](https://image.roku.com/ZHZscHItMTc2/channel-builder-2-cba-selection.png)
  ...
  2. If customers are required to sign in to your app, upload your [sign-in and sign-out test scripts](https://developer.roku.com/docs/developer-program/certification/cert-tests/authenticated-cert-testing.md#writing-automated-sign-in-and-sign-out-scripts) (unauthenticated apps can skip this step).
...
**Certification requirements** | Provides a link to the section in the [Certification Criteria](https://developer.roku.com/docs/developer-program/certification/certification.md) document, where the criterion in question is located.
...
## Publishing an app
Once your public app has passed [Static Analysis](https://developer.roku.com/docs/developer-program/dev-tools/static-analysis-tool/static-analysis-tool.md) and [Channel Behavior Analysis](https://developer.roku.com/docs/developer-program/publishing/channel-publishing-guide.md#channel-behavior-analysis-window) testing, you can schedule it for publishing by following these steps:
...
#### Device installation
Once a public app is published to the Streaming Store, it is available for installation on Roku devices immediately using the app's vanity access code, which is specified in the [Properties window](https://developer.roku.com/en-gb/docs/developer-program/publishing/channel-publishing-guide.md#properties-window). To get the latest version of the app, users can do a System Update (Home > Settings > System > System update > Check now) or update the individual app (press the options key [*] on the app tile > Check for updates).
