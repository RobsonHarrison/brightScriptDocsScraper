When your app is tested and ready to deploy, you publish it using the Developer Dashboard. There are two types of app you can work with.
  * **[Beta app](https://developer.roku.com/dev/docs/channel-publishing-guide#beta-app-guidelines)** : A sharable app for testing without certification. A beta app is a temporary app for development purposes that cannot be published to the public Streaming Store.
  * **[Public app](https://developer.roku.com/dev/docs/channel-publishing-guide#public-app-guidelines)** : An app available for install from the Streaming Store. A public app must pass Roku's certification process and be reviewed by Roku before being published.
...
> In other words, when publishing an app, make sure that your own account is the **root account** associated with the app, and that your third-party developer has simply been granted the role of "Administrator" or "App management." Our document on [user access management in the Developer Dashboard](https://developer.roku.com/dev/docs/user-access-management) provides more information on root accounts and role-based permissions.
##
App creation overview
[](https://developer.roku.com/dev/docs/channel-publishing-guide#app-creation-overview)
When you create an SDK app, you'll have the option of setting the app as a beta or public app. For either type of app, you'll start by [packaging your Roku app](https://developer.roku.com/dev/docs/packaging-channels). Then follow the guidelines for creating a beta app or a public app.
...
###
Beta app guidelines
[](https://developer.roku.com/dev/docs/channel-publishing-guide#beta-app-guidelines)
...
####
Beta app rules
[](https://developer.roku.com/dev/docs/channel-publishing-guide#beta-app-rules)
Beta app usage is **limited** to its intended purpose of quality assurance, as follows:
  * **120 days.** A beta app can exist for only 120 days after you create it (using [Add App](https://developer.roku.com/dev/docs/channel-publishing-guide#create-an-app)). After the expiration of that period, the app is: (1) deleted and removed from your Developer Dashboard and (2) disabled for all users who have installed it.
...
[](https://developer.roku.com/dev/docs/channel-publishing-guide#beta-apps-vs-public-apps)
...
####
Creating a beta app
[](https://developer.roku.com/dev/docs/channel-publishing-guide#creating-a-beta-app)
To create and start testing a beta app, you will need to:
  1. [Package your Roku app](https://developer.roku.com/dev/docs/packaging-channels).
  2. [Create an app](https://developer.roku.com/dev/docs/channel-publishing-guide#create-an-app) (select **Beta**).
  3. [Upload the package](https://developer.roku.com/dev/docs/channel-publishing-guide#package-and-testing) associated with this app.
...
[](https://developer.roku.com/dev/docs/channel-publishing-guide#public-app-guidelines)
...
To ensure success in creating a Public app, you will need to:
  1. [Package your Roku app](https://developer.roku.com/dev/docs/packaging-channels).
  2. Create your design assets following [Design and User Experience Guidelines](https://developer.roku.com/dev/docs/design).
  3. Go through the [Pre-certification Check List](https://developer.roku.com/dev/docs/certification).
  4. [Create an app](https://developer.roku.com/dev/docs/channel-publishing-guide#create-an-app) (select **Public**).
  5. [Upload the package](https://developer.roku.com/dev/docs/channel-publishing-guide#package-and-testing) associated with this app.
  6. Follow the [App certification](https://developer.roku.com/dev/docs/certification) procedure.
  7. Publish your app (submit it for deployment) using the [Publishing an app](https://developer.roku.com/dev/docs/channel-publishing-guide#publishing-an-app) procedure.

##
Create an app
[](https://developer.roku.com/dev/docs/channel-publishing-guide#create-an-app)
...
| Field  | Description  |
| --- | --- |
| App name  | Enter a name for your app. The name may be a maximum of 30 characters, and it may include alphanumeric, special, and UTF-8 characters. This name will be displayed in your app's listing in the Streaming Store.
...
| Streaming Store countries  | Select the countries where your app will be distributed from the drop-down list. You can scroll through the list and select countries or enter the name of a country to filter the list as you type.

Only select countries where you have the rights to distribute your app's content. The "Rest of World" Streaming Store includes regions not serviced by other Streaming Stores.

Customers are associated with Streaming Stores based on their region when they created their Roku account. For example, if a customer creates their Roku account while in the United States and then moves to another region, they are still associated with the U.S. Streaming Store.  |
...
You can click the link to install the app on a Roku device linked to your developer account. In the **App Profile** page, you can enter a descriptive custom code (for example, "MyVideoApp").  |
| Type  | The type of app, which may be one of the following values:
...
##
App store listing
[](https://developer.roku.com/dev/docs/channel-publishing-guide#app-store-listing)
###
Listing setup
[](https://developer.roku.com/dev/docs/channel-publishing-guide#listing-setup)
...
| Made for kids?  |  If your app is made for kids you must select **Yes** and comply with the applicable children’s privacy laws and youth protection laws in the countries in which your app is published. When you select **Yes** , the **Category** field is automatically fixed to "Kids" and cannot be changed, and you cannot set the **App Age Rating** to "Adult". If you select **No** , you cannot set the **Category** to "Kids". Selecting "Yes" automatically enables the **kidsContent** parameter in calls to the  |
...
| App age rating  | Specify the age rating that best describes your app. The selected rating is displayed on the app's details page in the on-device Streaming Store. This provides parents with information to help them determine whether your app is appropriate for their children.  |
###
Streaming store assets
[](https://developer.roku.com/dev/docs/channel-publishing-guide#streaming-store-assets)
...
| Field  | Description  |
| --- | --- |
| App name and description  | Enter a localized app name, short on-device description, and long online description for each language selected:

  * **App name** : The localized name of your app based on the selected language. You can provide different localized app names for each language you support.
  * **App description (on-device)** : The short app description (300-character maximum) that is included in your app's details screen, which is displayed when customers browse the Streaming Store on their device and select your app.
  * **App description (online)** : The long app description (1,500-character maximum) that is included in your app's detail page on [channelstore.roku.com](https://channelstore.roku.com/) .
...
##
App properties
[](https://developer.roku.com/dev/docs/channel-publishing-guide#app-properties)
###
App profile
[](https://developer.roku.com/dev/docs/channel-publishing-guide#app-profile)
...
| Field  | Description  |
| --- | --- |
| App type  | Select the type of app you are creating:
...
###
Monetization setup
[](https://developer.roku.com/dev/docs/channel-publishing-guide#monetization-setup)
...
| Subscription Video on Demand (SVOD)  | My app offers subscriptions.  | The app must [implement Roku Pay](https://developer.roku.com/dev/docs/roku-pay) and adhere to all [Roku Pay certification requirements](https://developer.roku.com/dev/docs/roku-pay-requirements#rp-3-payment-requirements).  |
| Transactional Video on Demand (TVOD)  | My app offers one-time purchases (movie rentals/purchases, PPVs, sporting events, and other transactional content).  | The app must [implement Roku Pay](https://developer.roku.com/dev/docs/roku-pay) and adhere to all [Roku Pay certification requirements](https://developer.roku.com/dev/docs/roku-pay-requirements#rp-3-payment-requirements).  |
| TV Everywhere (TVE)  | My app requires a cable or satellite subscription to access content.  |   |
| Ad-supported  | My app displays video ads.  |  If you select this option, you must enter the URL path to an app-ads.txt file in the **Developer URL** field to secure your app's ad inventory. If you do not sell ads via third parties or run programmatic ads on your app's inventory, it's recommended that you include the following line in the app-ads.txt file: "# [App Name] does not authorize programmatic sellers". For more information, see the [IAB documentation](https://iabtechlab.com/ads-txt/).
![channel-builder-2-monetization-options-video-ads - roku400px](https://image.roku.com/ZHZscHItMTc2/channel-builder-2-monetization-options-video-ads.png)
The **Channel ID** and **Bundle ID** values identify an app to the publisher's ad partners in the advertising ecosystem. These values can also be found via the meta-tags on the app's Streaming Store page. Following IAB App-Ads guidelines, Roku uses the meta-tags `appstore:store_id` and `appstore:bundle_id` to provide channel ID information in the http header of an app's Streaming Store page.
Using the values from the illustration above as examples, the meta-tags would appear in the `<head>` section, as so: HTML
```
...
```
Consider a scenario where you want to identify your app in an OpenRTB 2.5 Bid Request. You could use the following sequence for example: JSON
```
...
```
The **Developer URL** field is also provided (and discoverable) in an app's Streaming Store page `<head>` section: HTML
```
...
```
 |
| Pay-to-install  | My app requires customers to pay to install it.  | If you’ve enrolled in the Roku Partner Payouts Program, select the cost to install your app from the **One-time payment price tier** drop-down list.
...
![channel-builder-2-monetization-pay-to-install - roku400px](https://image.roku.com/ZHZscHItMTc2/channel-builder-2-monetization-pay-to-install.png)  |
  3. Click **Save**.

##
Package and testing
[](https://developer.roku.com/dev/docs/channel-publishing-guide#package-and-testing)
...
[](https://developer.roku.com/dev/docs/channel-publishing-guide#generating-a-package-file)
...
| Field  | Description  |
| --- | --- |
| Upload your .pkg or .zip file  | Upload the package file (.pkg) file that you created with the [Development Application Installer](https://developer.roku.com/dev/docs/packaging-channels).  |
| App version  | Select the version number for your app.  |
| Minimum firmware  | Roku supports multiple [package file formats](https://developer.roku.com/dev/docs/packaging-channels). Each package format requires a different minimum firmware version to be run on Roku devices, based on when the format was introduced to the OS. Enter a minimum firmware version that is equal to or greater than the one required for the package format used by your app.  |
  3. Click **Save**.

###
Deep linking
[](https://developer.roku.com/dev/docs/channel-publishing-guide#deep-linking)
To validate that your app is handling deep link requests properly, provide at least one set of [deep linking parameters](https://developer.roku.com/dev/docs/implementing-deep-linking) for each different [media type](https://developer.roku.com/dev/docs/implementing-deep-linking#understanding-deep-linking-parameters) in your app. For example, if your app contains movies and TV episodes, include deep linking parameters for at least one movie and one episode.
...
| Content ID  | Enter the [content ID](https://developer.roku.com/dev/docs/implementing-deep-linking) of the item (this should be the same as the **PlayId** in the app's [search feed](https://developer.roku.com/dev/docs/search-feed)).  |
...
###
Static Analysis
[](https://developer.roku.com/dev/docs/channel-publishing-guide#static-analysis)
...
| Severity  |  The type of message: error, warning, or info.
...
| Certification Requirement  | Provides a link to any related certification requirements in the [Certification Criteria](https://developer.roku.com/dev/docs/certification) document.  |
For additional usage details and operational information, including a list of the tests conducted, see the [Static Analysis Tool](https://developer.roku.com/dev/docs/static-analysis-tool) documentation.
###
Test credentials
[](https://developer.roku.com/dev/docs/channel-publishing-guide#test-credentials)
...
[](https://developer.roku.com/dev/docs/channel-publishing-guide#app-behavior-analysis)
Developers of SVOD, AVOD, and free apps can use the self-serve App Behavior Analysis tool to verify that their apps meet [performance](https://developer.roku.com/dev/docs/certification) and [deep linking](https://developer.roku.com/dev/docs/certification) certification requirements before submitting them for certification. This allows developers to get instant feedback on whether their apps meet specific deep linking and performance requirements, which reduces the wait time between submitting and publishing apps.
> To run the app launch performance test, your app must have the required [AppLaunchComplete signal beacon](https://developer.roku.com/dev/docs/measuring-channel-performance).
...
![channel-builder-2-cba-selection.png - roku600px](https://image.roku.com/ZHZscHItMTc2/channel-builder-2-cba-selection.png)
  2. If customers are required to sign in to your app, upload your [sign-in and sign-out test scripts](https://developer.roku.com/dev/docs/authenticated-cert-testing) (unauthenticated apps can skip this step).
...
| Certification requirements  | Provides a link to the section in the [Certification Criteria](https://developer.roku.com/dev/docs/certification) document, where the criterion in question is located.  |
| Actions  |  Displays information, script, and screenshot icons that you can click to learn more about the test:
...
##
Publishing an app
[](https://developer.roku.com/dev/docs/channel-publishing-guide#publishing-an-app)
Once your public app has passed [Static Analysis](https://developer.roku.com/dev/docs/static-analysis-tool) and [App Behavior Analysis](https://developer.roku.com/dev/docs/channel-publishing-guide) testing, you can schedule it for publishing by following these steps:
...
###
Post-publishing
[](https://developer.roku.com/dev/docs/channel-publishing-guide#post-publishing)
...
[](https://developer.roku.com/dev/docs/channel-publishing-guide#streaming-store-categories)
...
[](https://developer.roku.com/dev/docs/channel-publishing-guide#other-streaming-store-updates)
...
####
Roku Search
[](https://developer.roku.com/dev/docs/channel-publishing-guide#roku-search)
Apps and their individual content items are typically added to Roku Search within 24 hours of the app being published.
####
Device installation
[](https://developer.roku.com/dev/docs/channel-publishing-guide#device-installation)
Once a public app is published to the Streaming Store, it is available for installation on Roku devices immediately using the app's vanity access code, which is specified in the [Properties window](https://developer.roku.com/dev/docs/channel-publishing-guide#app-properties). To get the latest version of the app, users can do a System Update (Home > Settings > System > System update > Check now) or update the individual app (press the options key [*] on the app tile > Check for updates).
##
Rolling back an app to the previous release
[](https://developer.roku.com/dev/docs/channel-publishing-guide#rolling-back-an-app-to-the-previous-release)
...
[](https://developer.roku.com/dev/docs/channel-publishing-guide#updating-an-existing-app)
