...
![rendezvous-graph](https://image.roku.com/ZHZscHItMTc2/rendezvous-graph.png)
...
## Threading and observer callbacks
  ...
  * Any callbacks set up on a Task node's fields will be executed by the Render thread if the node is owned by the Render Thread. For a Task thread to respond to the setting of such a node's fields, use the message port form of `observeField()` and wait for an [roSGNodeEvent](https://developer.roku.com/docs/references/brightscript/events/rosgnodeevent.md) on that port. For nodes owned by the Task thread itself, the callback functions are set to call back directly to the Task thread.
...
> There is no advantage to calling the Garbage Collector frequently. For more information refer to [RunGarbageCollector](https://developer.roku.com/docs/references/brightscript/language/global-utility-functions.md#rungarbagecollector-as-object).
...
## Using getScene()
getScene() is a useful function to avoid having to store parent pointers. It gets the Node's root Scene, and from there you can search down for the Nodes you want to access without storing any pointers. More for information about getScene(), refer to [getScene() as roSGNode](https://developer.roku.com/docs/references/brightscript/interfaces/ifsgnodechildren.md#getscene-as-rosgnode).
## Image Sizing
