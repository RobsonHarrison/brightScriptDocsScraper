...
To link transactional content with Roku Pay, you create products and purchase options in the Developer Dashboard. With the TVOD model, products only need to be created for each [product category](https://developer.roku.com/docs/developer-program/roku-pay/quickstart/product-catalog.md#creating-products) (video content, audio content, or game token). For example, if you plan on offering movie rentals, you only need to create a single product that has the video category.
