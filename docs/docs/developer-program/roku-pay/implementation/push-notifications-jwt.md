...
> See [Setting up Roku Pay web services](https://developer.roku.com/docs/developer-program/roku-pay/quickstart/setting-up-web-services.md#push-notification-url) for how to add production and test push notification endpoints and enable them to receive secured Roku Pay push notifications.
...
Roku Pay push notifications use the JavaScript Object Signing and Encryption (JOSE) framework, which relies on a JSON Web Token (JWT) to deliver messages securely. The HTTP body of a Roku Pay push notification, which adheres to the [JWS compact serialization specification](https://datatracker.ietf.org/doc/html/rfc7515#section-3.1), includes the following JSON structures:
  * **JOSE header**. Specifies the key ID, which indicates the [public key](https://developer.roku.com/en-gb/docs/developer-program/roku-pay/implementation/push-notifications-jwt.md#public-keys) to be used to verify the signature and the signing algorithm required to decode the public key.
...
To receive the contents of Roku Pay push notifications, publishers must do the following:
  ...
  * Decode each string.
  * Use the [public key](https://developer.roku.com/en-gb/docs/developer-program/roku-pay/implementation/push-notifications-jwt.md#public-keys) to verify the signature string against the contents of the period-concatenated header and payload strings.
  * [Decode the **x-Roku-message** JWT claim within the payload to get the contents of the notification](https://developer.roku.com/en-gb/docs/developer-program/roku-pay/implementation/push-notifications-jwt.md#decoding-the-x-roku-message-jwt-claim-to-get-the-push-notification-message).
...
> See the [Roku Pay push notifications reference](https://developer.roku.com/docs/developer-program/roku-pay/implementation/push-notifications.md) for more information on the contents of the different Roku Pay push notification messages.
...
## Testing
Prior to enabling JWT signature authentication on a production endpoint, publishers can configure a test endpoint in the [Developer Dashboard](https://developer.roku.com/api/settings) and verify whether it can receive and process the JWT/JWS-secured messages. See [Setting up Roku Pay web services](https://developer.roku.com/docs/developer-program/roku-pay/quickstart/setting-up-web-services.md#test-configuration-for-push-notification) for more information.
In addition, publishers can manually send a test JWT/JWS-secured message with a generic payload to the configured test push notification endpoint. This enables publishers to verify that their test endpoint can receive a JWT/JWS-secured message without generating Roku Pay transactions. See [Setting up Roku Pay web services](https://developer.roku.com/docs/developer-program/roku-pay/quickstart/setting-up-web-services.md#notify-test-message-to-partner) for more information.
## Sample
