...
> All apps offering subscriptions must implement Enhanced Subscription Recovery to pass [certification](https://developer.roku.com/docs/developer-program/roku-pay/roku-pay-requirements.md#rp-4-authentication-and-entitlement-requirements).
## Overview
...
**App updates required**
  1. [Enable Enhanced Subscription Recovery](https://developer.roku.com/en-gb/docs/developer-program/roku-pay/subscription-recovery/subscription-on-hold.md#enabling-enhanced-subscription-recovery) for your **beta app** in the Developer Dashboard for testing the integration.
  2. [Perform entitlement checks using the Roku Pay APIs](https://developer.roku.com/en-gb/docs/developer-program/roku-pay/subscription-recovery/subscription-on-hold.md#entitlement-checks) to check whether a subscription is current, in recovery (in 3-day grace period), on hold, or canceled.
  3. [Implement the ChannelStore DoRecovery API](https://developer.roku.com/en-gb/docs/developer-program/roku-pay/subscription-recovery/subscription-on-hold.md#dorecovery-api) to display the Roku Pay subscription renewal dialog in your app when a customer selects content, navigates to or lands on a specific screen, or upon other specific interactions. This dialog notifies customers to update their MOP when auto-renewal fails..
  4. [Test your Enhanced Subscription Recovery integration](https://developer.roku.com/en-gb/docs/developer-program/roku-pay/subscription-recovery/subscription-on-hold.md#subscription-recovery-testing).

**Backend updates required**
  5. [Ingest and process additional push notifications](https://developer.roku.com/en-gb/docs/developer-program/roku-pay/subscription-recovery/subscription-on-hold.md#push-notifications) sent by Roku Pay as the subscription recovery state changes from in-grace, on-hold, and recovered.

**App publishing and enabling enhanced recovery**
  6. Once you have successfully completed and tested the Enhanced Subscription Recovery integration, you can [publish the updated **public** version of your app](https://developer.roku.com/docs/developer-program/publishing/channel-publishing-guide.md#updating-an-existing-channel), and then [Enable Enhanced Subscription Recovery](https://developer.roku.com/en-gb/docs/developer-program/roku-pay/subscription-recovery/subscription-on-hold.md#subscription-recovery-settings) for it.

### Enabling enhanced subscription recovery
Use the [**Subscription recovery** page](https://developer.roku.com/docs/developer-program/roku-pay/subscription-recovery/settings.md) in the Developer Dashboard to enable Enhanced Subscription Recovery for your app.
...
For more information on enabling Enhanced Subscription Recovery for your app, see [Subscription recovery settings](https://developer.roku.com/docs/developer-program/roku-pay/subscription-recovery/settings.md).
### Entitlement checks
You must use the Roku Pay APIs to check whether a subscription is current, in recovery (in 3-day grace period), on hold, or canceled. The [ChannelStore API](https://developer.roku.com/docs/references/scenegraph/control-nodes/channelstore.md#getallpurchases) is used to check the subscription status client-side upon app launch and then block access to content based on the results; the [Roku Pay web service APIs](https://developer.roku.com/docs/developer-program/roku-pay/implementation/roku-web-service.md) are used server-side for regular nightly syncs to update the publisher's entitlement service.
...
Use the **subscription-recovery** test API to manually force subscriptions into different states (active, in-grace period, on-hold, passively canceled, and recovered), which helps expedite the testing of your [subscription recovery](https://developer.roku.com/docs/developer-program/roku-pay/subscription-recovery/subscription-on-hold.md) integration.
