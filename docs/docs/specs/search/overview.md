...
To integrate into Roku Search, apps must provide the URL of their [search feed](https://developer.roku.com/docs/specs/search/search-feed.md), which is a [JSON](https://developer.roku.com/docs/specs/search/search-feed.md) file that contains a unique ID and metadata for each video in the app's catalog. The search feed must conform to Roku's schemas and be validated by Roku. Once the feed passes validation, content metadata is added to the Roku Search master database.
