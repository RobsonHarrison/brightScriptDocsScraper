...
To help developers test their channels, Roku provides a suite of tools to ensure your channel complies with the certification criteria. The test suite includes the following tools:
  * [Static Analysis tool](https://developer.roku.com/en-gb/docs/developer-program/certification/cert-tests/certification-testing.md): Checks the channel's code for certification-related errors.
  * [Channel Behavior Analysis tool](https://developer.roku.com/developer-program/publishing/channel-publishing-guide.md#channel-behavior-analysis-window): For SVOD, AVOD, and free channels, verifies channel performance and deep linking meets certification requirements.
  * [Test automation software](https://developer.roku.com/en-gb/docs/developer-program/certification/cert-tests/certification-testing.md): Enables developers to write and execute automated test cases, including channel purchasing, performance, deep linking, and other certification criteria.

Developers are expected to use these tools, the [certification criteria](https://developer.roku.com/docs/developer-program/certification/certification.md##certification-criteria-1), and the list of certification tests in this document to guide internal quality assurance testing before [submitting an app to Roku for review](https://developer.roku.com/docs/developer-program/certification/certification.md#publishing-your-channel).
## **1. Advertising**
**1.1** Apps that include video advertising must comply with all the integration requirements listed in [Roku advertising requirements](https://developer.roku.com/docs/developer-program/advertising/ad-requirements.md) document.
**Tests** : App is meeting the criteria specified in 1.1.
**Expected Result** : All ads are served via the [Roku Advertising Framework (RAF)](https://developer.roku.com/docs/developer-program/advertising/roku-advertising-framework.md) whether the channel is using the Roku default ad server or a third-party ad server (for example, Google DoubleClick for Publishers [DFP]. Freewheel, and so on). Ads render as expected on all Roku devices.
**RAF 1.1** Apps must integrate the [Roku Advertising Framework (RAF)](https://developer.roku.com/docs/developer-program/advertising/roku-advertising-framework.md) for all ads without modifying, obstructing, or disabling RAF functionality in any way. Replays of live broadcast streams are exempt from this requirement, unless dynamic ad insertion is used to insert new ads.
...
**RAF 1.3** Apps in the U.S. Roku Streaming Store only that are not child-directed must support Roku ad tracking by calling the [enableAdMeasurements()](https://developer.roku.com/docs/developer-program/advertising/raf-api.md#enableadmeasurementsenabled) method and passing the required content metadata within the following methods: [setContentGenre()](https://developer.roku.com/docs/developer-program/advertising/raf-api.md#setcontentgenregenres-as-string-kidscontent-as-boolean), [setContentId()](https://developer.roku.com/docs/developer-program/advertising/raf-api.md#setcontentgenregenres-as-string-kidscontent-as-boolean), and [setContentLength()](https://developer.roku.com/docs/developer-program/advertising/raf-api.md#setcontentlengthlength-as-integer). Optionally, channels may use the [setNielsenGenre API](https://developer.roku.com/docs/developer-program/advertising/raf-api.md#IntegratingtheRokuAdvertisingFramework-set-nielsen-genre) to pass specific Nielsen Genre granularity and the [setNielsenChannelId API](https://developer.roku.com/docs/developer-program/advertising/raf-api.md#setnielsenchannelidid-as-string) for those who specify a custom Nielsen Channel ID. The [enableAdMeasurements](https://developer.roku.com/docs/developer-program/advertising/raf-api.md#enableadmeasurementsenabled) method deprecates the [enableNielsenDAR](https://developer.roku.com/docs/developer-program/advertising/raf-api.md#nielsen-dar) API; therefore, do not use the [enableNielsenDAR](https://developer.roku.com/docs/developer-program/advertising/raf-api.md#nielsen-dar) API.
**Tests** :
  1. Get a test ad that contains trackers for Nielsen DAR and Comscore vCE. The method for getting the test ad varies based on whether you are implementing ads client-side via RAF or server-side:
     * Client-side: Call the [setAdUrl](https://developer.roku.com/docs/developer-program/advertising/raf-api.md#setadurlurl-as-string) method with no parameters.
     * Server-side: Email adsupport@roku.com with a request for an ad request URL, and then traffic the ad request URL on your ad server.
...
**Expected Result** : The number of ads in the ad break are displayed during the ad break using the standard Roku-branded label channellied by RAF.
...
**ADS 1.2** Apps that have an inventory relationship with Roku must meet the advertising terms specified in all applicbale agreements. See [Video Advertisements](https://developer.roku.com/docs/features/monetization/video-advertisements.md) for more information.
...
**Tests** : RIDA and LAT values may be passed on ad server requests client-side via RAF or server-side. Use one of the following tests corresponding to how you pass RIDA and LAT values in ad server requests:
  * Client-side via RAF:
    * Place [the URL parameter macros populated by RAF](https://developer.roku.com/docs/developer-program/advertising/integrating-roku-advertising-framework.md#url-parameter-macros) in ad request urls to populate the RIDA and LAT in the appropriate spot.
    * [Sideload the channel](https://developer.roku.com/docs/developer-program/publishing/packaging-channels.md#step-1-install-or-sideload-channel-on-a-roku-device), [Telnet into the Roku device](https://developer.roku.com/docs/developer-program/debugging/debugging-channels.md), and then [enable RAF debug mode](https://developer.roku.com/docs/developer-program/advertising/raf-api.md#setdebugoutputenabled-as-boolean).
    * Observe that RAF requests a test url and dispatches trackers on sideloaded channels when [setAdUrl](https://developer.roku.com/docs/developer-program/advertising/raf-api.md#setadurlurl-as-string) is passed an empty value.
  * Server-side: For server-side ad requests, the ad server's runtime macros are typically used to pass the RIDA and LAT values in the ad requests instead of the RAF macros. In this case, the RIDA and LAT values are usually passed to the ad server via Roku API calls.
    * Check your ad server for the runtime macros to be used to pass the RIDA and LAT values. Email adsupport@roku.com for help with this process.
    * Call the [GetRida()](https://developer.roku.com/docs/references/brightscript/interfaces/ifdeviceinfo.md#getrida-as-string) and [IsRidaDisabled()](https://developer.roku.com/docs/references/brightscript/interfaces/ifdeviceinfo.md#isridadisabled-as-boolean) methods to pass the RIDA and LAT values to your ad server, respectively.
...
**ADS 3.1** Apps must pass their Roku channel ID in ad server requests to Roku. See the [roChannelInfo.getId() function](https://developer.roku.com/docs/references/brightscript/interfaces/ifchannelinfo.md#getid-as-string) and the [ROKU_ADS_APP_ID URL parameter macro populated by RAF](https://developer.roku.com/docs/developer-program/advertising/integrating-roku-advertising-framework.md#url-parameter-macros).
**Tests** : The Roku channel ID may be passed on ad server requests client-side via RAF or server-side. Use one of the following tests corresponding to how you pass Roku channel ID values in ad server requests:
  * Client-side via RAF:
    * Place the [ROKU_ADS_APP_ID URL parameter macro populated by RAF](https://developer.roku.com/docs/developer-program/advertising/integrating-roku-advertising-framework.md#url-parameter-macros) in ad request urls to populate the Roku channel ID in the appropriate spot.
    * [Sideload the channel](https://developer.roku.com/docs/developer-program/publishing/packaging-channels.md#step-1-install-or-sideload-channel-on-a-roku-device), [Telnet into the roku device](https://developer.roku.com/docs/developer-program/debugging/debugging-channels.md), and then [enable RAF debug mode](https://developer.roku.com/docs/developer-program/advertising/raf-api.md#setdebugoutputenabled-as-boolean).
    * Observe that RAF requests a test url and dispatches trackers on sideloaded channels when [setAdUrl](https://developer.roku.com/docs/developer-program/advertising/raf-api.md#setadurlurl-as-string) is passed an empty value.
  * Server-side: For server-side ad requests, the ad server's runtime macros are typically used to pass the Roku channel ID in the ad requests instead of the RAF macros. In this case, the Roku channel ID is usually passed to the ad server via Roku API calls.
    * Check your ad server for the runtime macros to be used to pass the Roku channel ID. Email adsupport@roku.com for help with this process.
    * Call the [roChannelInfo.getId() method](https://developer.roku.com/docs/references/brightscript/interfaces/ifchannelinfo.md#getid-as-string) methods to pass the Roku channel ID to your ad server.
...
**ADS 3.2** Apps must use the Roku-generated device user agent in all server-side ad requests. See the [RAF integration guide](https://developer.roku.com/docs/developer-program/advertising/integrating-roku-advertising-framework.md#3-user-agent-requirements) for more details.
**Tests** :
...
To verify the monetization model for the channel:
  ...
  2. From within the channel, purchase any form of content.
  3. Verify the billing flow works properly for the entire purchase.
  4. From the channel, rent any form of content.
...
**Expected Result** : Apps that have integrated Instant Signup are performing in accordance with the [implementation document](https://developer.roku.com/docs/developer-program/discovery/instant-signup.md).
...
**Tests** :
  1. Navigate through the menus within the channel.
  2. Verify no content is being made available to outside channels.
  3. Verify no outside, third-party channel's content is available within the channel.
...
**Tests** :
  ...
  6. Roku Pay displays a "You're already subscribed to the channel" dialog.
...
**Tests** :
  1. Create a test Roku customer account.
  2. Activate and link Roku device to the account.
  3. Install and sign up to the channel
...
**Expected Result** : The channel handles multiple purchases properly.
...
**RP4.3** Apps must automatically entitle content or subscriptions purchased through Roku Pay across all devices tied to the purchasing Roku account. Apps can use the [getPurchases](https://developer.roku.com/docs/references/scenegraph/control-nodes/channelstore.md#getuserdata) API can upon launch to return the transactionID for an active subscription, and they can use an entitlement server to look up an account via a call to the [validate-transaction API](https://developer.roku.com/docs/developer-program/roku-pay/implementation/roku-web-service.md#validate-transaction).
**Tests** :
  1. Select 'Subscribe' button or navigate to content that requires subscription.
  2. Select 'Share'/'Don't share' option (if applicbale).
...
**Expected Result** : On channel launch a transactionID is returned for an active subscription.
...
**Tests** :
  1. Add the channel in testing to your Roku player.
  2. Navigate to the channel on the home screen and launch it.

**Expected Result** : App is responsive to user navigation, find, and playback of content at a reasonable speed.
**3.2** Apps must launch to a **fully rendered** home screen within 15 seconds. A signal beacon must be added to the channel to measure launch times. The debug console can then be used to verify that the channel's launch time is meeting this requirement. See [Measuring channel performance](https://developer.roku.com/docs/developer-program/performance-guide/measuring-channel-performance.md) on how to measure channel launch times.
**Tests** :
  1. Select your channel from the Roku home screen.
  2. Use the [BrightScript console (port 8085)](https://developer.roku.com/docs/developer-program/debugging/debugging-channels.md) to check the channel launch time.
  3. To retest, reboot your device and wait 30 seconds before testing again.

**Expected Result** : App is launched within 15 seconds of being selected from the Roku home screen. If you need to modify your application to meet this requirement, you can retest it by uninstalling your channel, rebooting your Roku device, and reinstalling the channel.
...
**Expected Result** : Transition time is within 3 seconds.
...
**Tests** :
  1. Navigate channel UI with Roku remote button presses and time the response.
  2. Navigate channel UI tile-to-tile and time the response.

**Expected Results** : A response to a remote button press or tile-to-tile navigation do not exceed 250 milliseconds.
...
The debug console can be used to verify that video start times are compliant. See [Measuring channel performance](https://developer.roku.com/docs/developer-program/performance-guide/measuring-channel-performance.md) for more information.
...
**Expected Result** : Content will start playing within 8 seconds of initiation. If you need to modify your application to meet this requirement, you can retest it by uninstalling your channel, rebooting your Roku device, and reinstalling the channel.
**3.7** The channel's file size must be 4 MB or less.
**Tests**
  1. The file size of your channel is 4MB or less.

**Expected Result** : File size of the channel is no larger than 4MB.
## **4. App operation**
**4.1** App updates are prohibited from requiring reactivation/re-linking/re-login, and must persist saved data.
**Tests** :
  1. Remove all versions of the channel from the Roku player.
  2. Reboot Roku device.
  3. Install the currently published version of the channel from the Roku Streaming Store.
  4. Launch the channel and link to a valid production account.
  5. Exit the channel.
...
**4.2** Apps that require a user to log in and that have streamed more than an average of 1 million hours per month over the last three months must integrate [Automatic Account Link](https://developer.roku.com/docs/developer-program/authentication/universal-authentication-protocol-for-single-sign-on.md). This requirement is also applicbale to new channels projected to reach the specified streaming hours threshold shortly after launch.
**Tests** :
  1. Create a test user Roku customer account.
  2. Activate and link Roku device to the account.
  3. Install and sign up to the channel.
  4. Link another Roku device to the account.
  5. Try to load the channel. It should sign the test user in automatically.

**Expected Result** : User can be authenticated and sign in successfully from other devices.
**4.3** Apps that require authentication (SVOD, TVE, and other subscription services) must use the [Roku Event Dispatcher](https://developer.roku.com/docs/developer-program/discovery/search/prioritizing-authenticated-channels-in-roku-search.md) to communicate authentication status.
**Tests** :
...
**4.4** Roku [reserves use of the Options](https://developer.roku.com/docs/developer-program/design/masterui.md) ("*") button during video playback for the sole purpose of Roku system displays. Apps may use the Options button for additional in-channel options while UI components are displayed on top of the video.
**Tests** :
  1. Start playing a video.
  2. Press "*" key on your remote control during playback.

**Expected Result** : During video playback with no obscuring UI components, the "*" button is not used for any additional in-channel options.
**4.5** Apps are prohibited from overriding or interfering with Roku's system screensaver. See [Roku’s Screensaver Policy](https://developer.roku.com/docs/developer-program/media-playback/screensavers.md).
**Tests** :
...
**4.6** Apps must directly return the user to the previous screen and/or state when the [back button](https://developer.roku.com/docs/developer-program/design/remote-control-buttons.md) on the Roku remote is pressed. When the back button is pressed on the channel's home screen, channels must exit the channel and return the user to the Roku home screen. Apps may display a single confirmation dialog immediately before the user exits the channel—and then return the user to the Roku home screen upon receiving confirmation that they want to leave the channel.
**Tests** :
  1. Launch the channel and navigate to content and menus.
  2. Press back button on your remote.
  3. Navigate to channel homescreen.
  4. Press back button on your remote until you land on Roku's Home screen.

**Expected Result** : All back buttons behave properly.
**4.7** Apps must display thumbnails during [trick play](https://developer.roku.com/docs/developer-program/media-playback/trick-mode/trick-mode.md) for VOD content longer than 15 minutes.
**Tests** :
...
**Expected Result** : Thumbnails are displayed during VOD trick play.
**4.8** Apps must comply with [all applicbale accessibility laws](https://docs.roku.com/published/channelaccessibility/en/us) and global settings related to accessibility and user experience. Apps must provide [closed captions](https://developer.roku.com/docs/developer-program/media-playback/closed-caption.md) and audio descriptions for content where required by law. If including closed captions, the channel must follow the user global settings for closed captioning, and support the following closed captioning settings in the Options menu: On, Off, On instant replay, and On mute (Roku TVs only). For VOD content, channels must synchronize the captions with the audio.
Apps must adhere to [Roku’s autoplay policy](https://developer.roku.com/docs/developer-program/media-playback/autoplay.md).
**Tests** :
...
**4.9** Apps must rewind between 10 to 25 seconds when the [instant replay button](https://developer.roku.com/docs/developer-program/design/remote-control-buttons.md#instant-replayskip-back) on the Roku remote is pressed.
**Tests** : Instant replay should rewind between 10 to 25 seconds.
**Expected Result** : Video rewinds between 10 to 25 seconds.
**4.10** Apps must implement [bookmarking](https://developer.roku.com/docs/developer-program/media-playback/bookmarking.md) for all VOD content longer than 15 minutes. Apps must store bookmarks for a minimum of 30 days.
**Tests** :
  ...
  4. Navigate to the channel on the Home screen and launch it.
  5. Navigate back to the video that was played in step #1.
  6. Select the resume option. Verify that the clip begins to play at a close channelroximation of where it last played.
...
**4.11** Apps that have streamed more than an average of 5 million hours per month over the last three months must implement all [voice controls](https://developer.roku.com/docs/developer-program/media-playback/voice-controls/transport-controls.md) that are supported on the Roku platform. Apps must implement proper [error handling](https://developer.roku.com/docs/developer-program/media-playback/voice-controls/transport-controls.md#error-handling) for unsupported voice commands. These requirements are also applicbale to new channels projected to reach the specified streaming hours threshold shortly after launch.
**Tests** :
  1. Launch the channel
...
**4.12** Apps must use [Roku voice keyboards](https://developer.roku.com/docs/references/scenegraph/dynamic-voice-keyboard-nodes/dynamic-keyboard-base.md) for [email](https://developer.roku.com/docs/references/scenegraph/dynamic-voice-keyboard-nodes/dynamic-keyboard.md), [PIN](https://developer.roku.com/docs/references/scenegraph/dynamic-voice-keyboard-nodes/dynamic-pinpad.md), and [password](https://developer.roku.com/docs/references/scenegraph/dynamic-voice-keyboard-nodes/dynamic-keyboard.md) entry.
**Tests** :
  1. Launch the channel
  2. If channel sign-in is required, a [dynamic voice keyboard](https://developer.roku.com/docs/references/scenegraph/dynamic-voice-keyboard-nodes/dynamic-keyboard.md) or [dynamic custom voice keyboard](https://developer.roku.com/docs/references/scenegraph/dynamic-voice-keyboard-nodes/dynamic-custom-keyboard.md) is used for entering an email address and password.
  3. If PIN entry is required (for example, when entering a parental access code), a [dynamic voice PinPad](https://developer.roku.com/docs/references/scenegraph/dynamic-voice-keyboard-nodes/dynamic-pinpad.md) or [dynamic custom voice keyboard](https://developer.roku.com/docs/references/scenegraph/dynamic-voice-keyboard-nodes/dynamic-custom-keyboard.md) is used.
...
**Tests** :
  ...
  3. In your browser, launch [Roku Deep linking Tester tool](https://devtools.web.roku.com/DeepLinkingTester/). Follow the instructions to select the channel ID and channel name.
...
**Expected Result** : Content can be launched through deep links properly based on mediaType.
**5.2** Apps must implement [Direct to Play](https://developer.roku.com/docs/developer-program/discovery/direct-to-play.md) to support direct voice playback commands for launching and playing content.
**Tests** :
  ...
  4. Send an [External Control Protocol (ECP)](https://developer.roku.com/docs/developer-program/dev-tools/external-control-api.md) **input** command via cURL to your Roku device using the following syntax: curl -d '' '[http://<Roku](http://%3CRoku) Device IP Address>:8060/input?contentId=<_contentIdValue_ >&mediaType=<_mediaTypeValue_ >'
  5. Repeat test for unauthenticated and authenticated users.

**Expected Result** : Content is launched as specified for the following conditions:
...
**Tests** :
  1. Navigate through the menus within the channel.
  2. Ensure that within the channel there is no option to deep link into any third-party channels.

**Expected Result** : Your channel does not deep link into any third-party channels.
## **6. UI and graphics**
**6.1** Apps must have a non-zero version number. This number must be incremented for each build submitted and updated in the [manifest](https://developer.roku.com/docs/developer-program/getting-started/architecture/channel-manifest.md).
**Tests** :
  1. Navigate to the channel on the Roku device's Home screen and focus it.
  2. Select * (star) key on your remote to invoke the pop-up with channel information.
  3. Verify the version number is not zero.

**Expected Result** : Version number is greater than zero.
...
**Tests** :
  1. Navigate through the channel and examine for inappropriate content.

**Expected Result** : The Kids & Family category displays no inappropriate content.
...
**Tests** :
  1. Navigate through the channel and examine content for pornography.

**Expected Result** : Your public channel does not contain pornographic content.
