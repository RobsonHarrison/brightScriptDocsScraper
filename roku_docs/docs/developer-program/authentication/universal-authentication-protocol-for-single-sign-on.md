...
```
  ...
  * If the access token is invalid or there is no access token, [check for an access token in the Roku cloud](https://developer.roku.com/en-gb/docs/developer-program/authentication/universal-authentication-protocol-for-single-sign-on.md#checking-for-a-valid-access-token-in-the-roku-cloud). Use the [**roRegistrySection.delete()**](https://developer.roku.com/docs/references/brightscript/interfaces/ifregistrysection.md#deletekey-as-string-as-boolean) method to delete an invalid access token from the device registry:
...
```
  ...
  4. If the customer is signed out (or is signing up), display your app UI and have them [sign in](https://developer.roku.com/en-gb/docs/developer-program/authentication/universal-authentication-protocol-for-single-sign-on.md#signing-users-in).
...
You can test Automatic Account Link by sideloading your app and using the [billing test feature in the Developer Dashboard](https://developer.roku.com/docs/developer-program/roku-pay/testing/billing-testing.md).