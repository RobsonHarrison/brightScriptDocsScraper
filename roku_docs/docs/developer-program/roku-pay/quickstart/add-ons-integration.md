...
Integrating add-ons entails the following steps:
  1. Creating add-on products in the Developer Dashboard. A product represents a set of content (for example, a premium app or app package) or other features offered by your app. **You must upgrade to Product Catalog 2.0 to create add-ons** (see [Creating the Product Catalog](https://developer.roku.com/docs/developer-program/roku-pay/quickstart/product-catalog.md) for how to migrate the In-app purchase workflow in the Developer Dashboard to the new Product Catalog 2.0 experience).
...
## Updating the app
Publishers need to update their app's code to leverage the new **GetAllPurchases** , **GetCatalog** , **QueryPuchaseOptions** , and **DoOrder** ChannelStore APIs, which have been enhanced for supporting add-ons. These APIs are summarized as follows (See [Appendix A](https://developer.roku.com/en-gb/docs/developer-program/roku-pay/quickstart/add-ons-integration.md#appendix-a-catalog-20-apis) for the Add-on API reference):
