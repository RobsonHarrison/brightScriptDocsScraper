# roChannelStoreEvent
The roChannelStore sends an roChannelStoreEvent in response to a call to any of several Get* methods in [ifChannelStore](https://developer.roku.com/docs/references/brightscript/interfaces/ifchannelstore.md "ifChannelStore"). The following predicates indicate its valid event types:
## Supported methods
...
Returns an roList of roAssociativeArray items for the previous GET method invocation. The format of each roAssociativeArray item depends on which GETmethod was invoked. See [ifChannelStore](https://developer.roku.com/docs/references/brightscript/interfaces/ifchannelstore.md "ifChannelStore") for more details.
