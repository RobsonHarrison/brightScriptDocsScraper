...
### GetID() as Integer
#### Description
Returns a unique identifier that can be compared to the value returned by the [roSocketEvent.getSocketID()](https://developer.roku.com/docs/references/brightscript/events/rosocketevent.md#getsocketid-as-integer) method to match the underlying socket to receive the event.
#### Return Paramters
A unique ID.