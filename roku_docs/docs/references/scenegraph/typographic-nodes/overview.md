# Typography
The SceneGraph API includes the **Font** node class for specifying the font characteristics to be used by your application. Each node class that renders text on the screen display includes a `font` field to set the font characteristics for the node.
There is a default font supplied with the Roku OS. To use a different font, you must include it in the application package, such as in `pkg:/fonts/`. Then you can specify that this font be used as the font for a node class that renders text on the screen display in a couple of ways:
